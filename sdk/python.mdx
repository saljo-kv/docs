...
```
- **Parameters**:
  - `conversation_type` (ConversationType): The conversation entry type. One of `ConversationType.INPUT`, `ConversationType.OUTPUT`, or `ConversationType.SYSTEM`.
  - `field_name` (str): A non-empty field name to describe the value (e.g., "prompt", "response", "system_message").
  - `value` (Any): The value to record. Must not be `None`.
- **Returns**: None
