...
### set_input
```python
def set_input(value: Any, span_name: Optional[str] = None) -> None
```

- **Description**: Set custom attribute `netra.span.input` on a target span.

- **Parameters**:
  - `value` (Any): Input payload to record (string or JSON-serializable object)
  - `span_name` (Optional[str]): Optional. When provided, sets the attribute on the span registered with this name. Otherwise sets on the active span.
- **Returns**: None

### set_output
```python
def set_output(value: Any, span_name: Optional[str] = None) -> None
```

- **Description**: Set custom attribute `netra.span.output` on a target span.

- **Parameters**:
  - `value` (Any): Output payload to record (string or JSON-serializable object)
  - `span_name` (Optional[str]): Optional. When provided, sets the attribute on the span registered with this name. Otherwise sets on the active span.
- **Returns**: None
