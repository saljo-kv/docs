...
# Record batch
Netra.set_custom_events(batch_events)
```

</CodeGroup>

### Event Context Inheritance

Events automatically inherit session context:

<CodeGroup>

```python python
# Set base session context
Netra.set_session_id("session-123")
Netra.set_user_id("user-456")
Netra.set_custom_attributes("user_role", "admin")

# Events will inherit these attributes
Netra.set_custom_event("admin_action", {
...
1. **Event Not Showing Up**
   - Check if SDK is properly initialized
   - Verify session ID is set
   - Ensure event name is correctly formatted
   - Check for typos in attribute names

2. **Performance Issues**
   - Use batch event recording for high-frequency events
   - Avoid excessive attribute data
   - Consider event sampling for high-volume scenarios

3. **Data Inconsistency**
   - Review event attribute naming conventions
